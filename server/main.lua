...
local AllTokens = {} -- Semua Token Pemain (Anti Banyak Akun)
local PlayerTokens = {} --Token Player Saat Terhubung Ke Server
local DatabaseStuff = {} -- Masukkan Dalam Dabatase
local BannedTokens = {}   --Banned Tokens Dari Server
local AllSteams = {}     --Semua Steam Hex (Anti Banyak Akun)
local AllLicenses = {}    -- Semua License Pemain (Anti Banyak Akun)
local MultiAcc = {
    Token = false,
    Hex = false,
    Licensess = false
...
RegisterServerEvent('Gusti_BanSystem:InsertMe')
AddEventHandler('Gusti_BanSystem:InsertMe', function()
...
Citizen.CreateThread(function() -- Ini Sangat-Sangat Lelah Tapi .... Saya Tidak Punya Ide Tentang Informasi
    while true do 
        Citizen.Wait(60000)
        Informations()
        Citizen.Wait(20000)
